...
public interface PostRepository extends CrudRepository<Post, Long> {
  ...
  @Query(value = "SELECT DISTINCT POST.post_id, POST.title, POST.text, POST.author, POST.email,"
      + " POST.published FROM POST JOIN (SELECT pk1.post_id FROM POST_KEYWORD as pk1 "
      ...
      + "AND k2.keyword_id = pk3.keyword_id ))) as found ON POST.post_id = found.post_id;", nativeQuery = true)
